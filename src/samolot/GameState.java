...
/** 
 * Klasa generujaca podstawowe elementy gry dla poziomu pierwszego 
...
public class GameState extends BasicGameState {
	...
	/** Inicjowanie klasy GameState wraz z ustawieniem poziomu pierwszego */
	@Override
	...
	public void update(GameContainer gc, StateBasedGame sbg, int t) throws SlickException {
		...
		if (LoadingState.timePassed > 7000 + LoadingState.random.nextInt(3000)) {
		...
		if (gc.getInput().isKeyDown(Input.KEY_UP)) {
			...
			LoadingState.yPlane -= 9;
		}
		
		if(LoadingState.yPlane > gc.getHeight() || LoadingState.yPlane < -LoadingState.plane.getHeight()) {
			LoadingState.gameOverText1 = "Przekroczy³eœ granice ekranu";
			LoadingState.gameOverText2 = "";
			sbg.enterState(50, new FadeOutTransition(), new FadeInTransition());
		}
		...
		if (LoadingState.levelTime <= 16) {
			if (LoadingState.points1 < 2) {
			sbg.enterState(50, new FadeOutTransition(), new FadeInTransition());
			} else {
				sbg.enterState(30, new FadeOutTransition(), new FadeInTransition());
			}
		}
	}
	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException {
		...
		LoadingState.uniFont.drawString(855, 940, "PUNKTY: " + LoadingState.points1);
		LoadingState.label.draw(1050, 924);
		LoadingState.uniFont.drawString(1075, 940, "POZIOM: " + level);
	}

	/** Przypisanie numeru identyfikacyjnego klasie GameState */
	@Override
	public int getID() {
		return 10;
	}
